...
const renderImages = async () => {
    ...
    window.totalImages = lazyLoadedData

    let index = JSON.parse(localStorage.getItem('index') || '0')
    if (!index) JSON.stringify(localStorage.setItem('index', 0))
    wrapper[0].innerHTML = ""
    for (let i = 0; i < 6; i++) {
        let imgIndex = (index + i) % lazyLoadedData.length
        let photo = lazyLoadedData[imgIndex]
...
export { renderImages, handleBlurryImages }